...
The [kernel sandbox](/linux-sandbox) is the foundation. Landlock (Linux) and Seatbelt (macOS) create an irrevocable allow-list at the kernel level. This is not a filter that inspects operations after the fact &mdash; it is a structural constraint that makes unauthorized operations impossible.
