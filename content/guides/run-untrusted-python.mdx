...
The script can read and write in the current directory. Everything else — `~/.ssh`, `~/.aws`, `~/.gnupg`, system configs — is blocked at the kernel level. The restriction is enforced by Landlock (Linux) or Seatbelt (macOS) and cannot be bypassed from inside the process.
