...
nono applies kernel-level restrictions (Landlock on Linux, Seatbelt on macOS) to a process running in your normal environment. The process sees your filesystem but can only access what you explicitly allow. There's no container image, no volume mounts, no network bridge.
...
- **macOS-native isolation** — Seatbelt enforcement, not a Linux VM
