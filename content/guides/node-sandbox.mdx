...
nono enforces kernel-level restrictions on the Node.js process before it starts. The sandbox is applied by the OS kernel (Landlock on Linux, Seatbelt on macOS) and cannot be loosened from inside the process. No `vm` module tricks, no V8 isolate escapes — the kernel denies the syscall directly.
...
# Run with injection
nono run --allow-cwd --proxy-credential openai -- node my_agent.js
```
