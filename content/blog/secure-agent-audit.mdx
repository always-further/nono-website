...
date: "2026-04-22"
description: "How nono records every action an AI agent makes in an append-only Merkle tree the agent itself cannot reach, signs the root of that tree with a private key, and lets anyone verify after the fact — with cryptographic proof — that the record was not forged, edited, or truncated."
author: "Luke Hinds"
...
![Title / hero image](/audit-record-arch-title.png)

*How nono records every capability decision an AI agent makes in an append-only Merkle tree the agent itself cannot reach, signs the root of that tree with a private key, and lets anyone verify after the fact — with cryptographic proof — that the record was not forged, edited, or truncated.*

---

## The problem with "trust me, bro" logs

If you run an autonomous coding agent on your machine — Claude Code, Codex, or any of the others — you are giving a language model permission to open files, run commands, touch your filesystem, and reach out to the network. That is the whole point. It is also a lot of power.

So: **what actually happened during that session?**

Most tooling hands you a log file. A log file is a story the program tells about itself. If the program is compromised — or if the agent has managed to write somewhere it shouldn't — the log becomes part of the attack surface. "I didn't touch `.env`" is not evidence when the same process that might have touched `.env` is the one writing the log entry that says so.
...
This post describes how [nono](https://nono.sh) does each of these, walks through the crypto well enough that "Merkle tree" and "inclusion proof" stop being buzzwords, and shows why the combination makes nono the strongest open-source audit trail available for agent execution today.

---

## nono's core architecture in one minute

nono runs untrusted agent commands inside an OS-enforced sandbox (Landlock on Linux, Seatbelt on macOS). The defining structural choice is **two processes**:
...
The root is one 32-byte value that cryptographically commits to every leaf underneath it. Change any leaf and the root changes. Truncate the tree and the root changes. Reorder the leaves and the root changes.

![merkle proof](/audit-record-arch-two.png)

### Inclusion proofs: one value, no full replay

Here is the quiet superpower. If I give you the Merkle root and then later claim "event #3 really was in the log" — I don't have to show you the whole log to prove it. I just have to show you:
...
This is called an **inclusion proof** or **audit proof**. It is the same trick Certificate Transparency uses to prove a TLS cert is in a public log without downloading the whole log. It is the same trick [Sigstore Rekor](https://docs.sigstore.dev/logging/overview/) uses to prove a signature was recorded.
...
This adds a second, complementary integrity property. The Merkle root proves "the set of events is exactly this." The chain proves "and they happened in exactly this order, one after another." An attacker who tries to delete, reorder, or splice events has to break both at once.

Every hash operation uses **domain separation** — a constant string mixed in so that a leaf hash can never be confused with an internal-node hash or a chain hash. nono tags every domain: `"nono.audit.event.alpha\n"`, `"nono.audit.chain.alpha\n"`, `"nono.audit.merkle.alpha\n"`. This is a small detail with a big payoff: it structurally prevents length-extension and type-confusion attacks between the three hashing contexts.
...
Every supervised session produces a directory under `~/.nono/audit/<session_id>/`:
...
## Binary identity: *what* actually ran
...
The signing key can come from a file, system keyring, 1Password, Apple Keychain, an environment variable, or any other trust-service-supported secret reference — the secrets never hit disk in plaintext.

The signature is over the Merkle root plus bound context. So to forge a session an attacker must:

- Rewrite `audit-events.ndjson` (easy if they have write access)
- *And* recompute a Merkle root they like
- *And* sign that root with the attestation key they do not have

The only degree of freedom an attacker without the key has is to delete the attestation bundle entirely — which is obvious on verification (a signed session missing its bundle is a bright red flag), not silent forgery.
...
Bring the threat model together. Suppose an attacker has full code execution as the sandboxed agent. What can they actually do to the record?

| Attack | What structurally stops it |
|---|---|
| Write to the audit file directly | Landlock / Seatbelt — the audit directory is not in the child's allowed set. The child literally cannot `openat` it. |
...
| Delete the session entirely | Global hash-chained ledger at `~/.nono/audit/ledger.ndjson` — missing session = broken chain. |
...
| Forge a new session entirely | Requires the attestation signing key, which lives in the operator's keystore, not the host's filesystem. |

The one attack this does **not** structurally prevent is a host attacker who has *both* arbitrary code execution as the operator *and* access to the signing key. That is an inherent limit of local signing; the answer is external anchoring (transparency logs, TSA timestamping, remote witnesses) — visible future work, not a silent gap.
...
The short answer: most "agent security" today is policy without proof. Rules get written, decisions get made, and you are trusted to believe the program's own narrative about what it did.

nono flips that. The sandbox decides policy at runtime; the audit trail makes every one of those decisions, every binary executed, and every filesystem change into cryptographic evidence. Three independent people — the operator who ran the agent, the engineer reviewing it next week, and an auditor six months from now — can all verify the same session and arrive at the same answer, without trusting each other and without trusting the host.

That is what a tamper-evident audit trail looks like. It's what agent execution has been missing. And it is fully open source.

Try it:

```
nono run --audit-integrity --audit-sign-key keystore://nono-audit -- claude
nono audit verify <session_id> --public-key-file nono-audit.pub
```

---

## Appendix: crypto specifics for the curious

- Hash function: SHA-256 everywhere
- Domain separators (RFC 6962 style):
  - `"nono.audit.event.alpha\n"` — leaf hashing
  - `"nono.audit.chain.alpha\n"` — per-event hash chain
  - `"nono.audit.merkle.alpha\n"` — internal Merkle nodes
  - `"nono.audit.session-digest.alpha\n"` — session digest for ledger
  - `"nono.audit.ledger.chain.alpha\n"` — global ledger chain
- Leaf prefix byte `0x00`, internal-node prefix byte `0x01` — prevents second-preimage via leaf/node confusion
- Attestation format: DSSE envelope wrapping an in-toto v1 statement
- Predicate type: `https://nono.sh/attestation/audit-session/alpha`
- Subject digest: the audit Merkle root, hex-encoded
- Global ledger locking: `flock(2)` advisory lock on `~/.nono/audit/ledger.lock` for safe concurrent finalization

# Documentation links:

- [nono audit documentation](https://nono.sh/docs/cli/features/audit)
- [nono security model](https://nono.sh/docs/cli/internals/security-model)
- [Certificate Transparency and Merkle trees](https://www.certificate-transparency.org/merkle-trees)
- [in-toto and DSSE](https://in-toto.io/)
