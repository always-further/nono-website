...
AI agents that write code, run commands, and interact with your filesystem need guardrails. Without them, a single hallucinated `rm -rf` or an unintended credential read can cause real damage. **nono** is a capability-based sandboxing system that uses [OS-enforced isolation](/linux-sandbox) (Landlock on Linux, Seatbelt on macOS) to make unauthorized operations structurally impossible.
