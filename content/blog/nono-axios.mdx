...
| Linux | `/tmp/ld.py` | Denied in typical project-scoped profiles; if `/tmp` is explicitly granted, network filtering still blocks payload download |
...
## Try it: sandbox your next npm install

```bash
brew install nono

# Sandbox npm install using the built-in node profile
nono run --profile node -- npm install

# More restrictive: block all network, allow only what's needed
nono run --read ~/.npm --allow ./node_modules --block-net -- npm install

# Sandbox a Node.js agent with network filtering and credential injection
nono run --allow-cwd --network-profile minimal \
  --proxy-credential openai \
  -- node my_agent.js
```

The built-in `node` profile includes the `node_runtime` paths (`~/.nvm`, `~/.npm`, etc.) so most installs work out of the box. If you want to discover exactly what a specific install needs, use `nono learn`:

```bash
# Trace an npm install to generate a profile
nono learn npm install
```

Review the output, save it as a profile, and every subsequent install runs with the same sandboxed restrictions. Postinstall hooks execute — but inside a kernel-enforced boundary where they can't reach your credentials, can't call unknown hosts, and can't write outside the allowed paths.
